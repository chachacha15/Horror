...
namespace Artngame.Oceanis.GPUParticles
{
    public class GPUParticleEmitter : MonoBehaviour
    {
        ...
        private void OnRenderObject()
        {
            ...
            if (oceanMat != null)
            {
                Texture oceanTex = oceanMat.GetTexture("_Displacement_c0");
                //computeShader.SetTexture(0, "oceanTex", oceanTex, 0);
                renderMaterial.SetTexture("oceanTex", oceanTex);

                //renderMaterial.SetVector("lengthScales", oceanMat.GetVector("lengthScales"));
                //renderMaterial.SetFloat("LengthScale0", oceanMat.GetFloat("LengthScale0"));
                //renderMaterial.SetFloat("LengthScale1", oceanMat.GetFloat("LengthScale1"));
                //renderMaterial.SetFloat("LengthScale2", oceanMat.GetFloat("LengthScale2"));
                //renderMaterial.SetFloat("_LOD_scale", oceanMat.GetFloat("_LOD_scale"));
            }
            ...
            if (enableEmission)
            {
                ...
                if (count > 0)
                {
                    ...
                    if (emissionShape == EmissionShape.Sphere)
                    {
                        ...
                        {
                            Texture oceanTex = oceanMat.GetTexture("_Displacement_c0");
                            computeShader.SetTexture(emitKernel, "oceanTex", oceanTex, 0);

                            //computeShader.SetVector("lengthScales", oceanMat.GetVector("lengthScales"));
                            //computeShader.SetFloat("LengthScale0", oceanMat.GetFloat("LengthScale0"));
                            //computeShader.SetFloat("LengthScale1", oceanMat.GetFloat("LengthScale1"));
                            //computeShader.SetFloat("LengthScale2", oceanMat.GetFloat("LengthScale2"));
                            //computeShader.SetFloat("_LOD_scale", oceanMat.GetFloat("_LOD_scale"));
                            if (oceanController != null)
                            {
                                computeShader.SetVector("lengthScales", oceanController.lengthScales);
                                computeShader.SetFloat("LengthScale0", oceanController.lengthScale0);
                                computeShader.SetFloat("LengthScale1", oceanController.lengthScale1);
                                computeShader.SetFloat("LengthScale2", oceanController.lengthScale2);
                            }
                           // computeShader.SetFloat("_LOD_scale", oceanMat.GetFloat("_LOD_scale"));
                        }
                    ...
                    //v0.1
                    else if (emissionShape == EmissionShape.Mesh && oceanMat != null)
                    {
                        Texture oceanTex = oceanMat.GetTexture("_Displacement_c0");
                        computeShader.SetTexture(0,"oceanTex", oceanTex, 0);
                    }
        ...
        private void DispatchUpdate()
        {
            if (timeScale > 0)
            {
                ...
                {
                    Texture oceanTex = oceanMat.GetTexture("_Displacement_c0");
                    computeShader.SetTexture(updateKernel, "oceanTex", oceanTex, 0);
                    if (oceanController != null)
                    {
                        computeShader.SetVector("lengthScales", oceanController.lengthScales);
                        computeShader.SetFloat("LengthScale0", oceanController.lengthScale0);
